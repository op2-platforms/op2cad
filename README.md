...
The current prefered way to work is to "Fork" the [main repository](https://github.com/op2-platforms/op2cad) to your own or enterprises GitHub account and clone this newly made repository to your C drive in a GitHub folder (C:\GitHub\op2cad). This way you can use absolute path dependent files (.prj, .prv) right away and get going with existing applications customization. Else, if you are working from multiple computers, you may prefer to build it inside a cloud folder; but you'll have to remake all the absolute path dependent files.
